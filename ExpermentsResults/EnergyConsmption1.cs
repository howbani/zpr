...
namespace ProZoneRouting.ExpermentsResults
{
    ...
    public class DoEnergyConsumptionExpermentRandomWithDistance
    {
        ...
        private List<Sensor> SelectNodesWithinDistance(double distance)
        {
            ...
            if (distance > (PublicParamerters.CommunicationRangeRadius * 1.2))
            {
                double maxDi = distance + (PublicParamerters.CommunicationRangeRadius / 2);
                double MinDi = distance - (PublicParamerters.CommunicationRangeRadius / 2);
