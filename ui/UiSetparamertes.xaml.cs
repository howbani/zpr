        ...
        public List<Experment> DoExperment()
        {
            ...
            for (int i = 0; i < expermentsNumber; i ++)
            {
                ...
                if (check_random.IsChecked == true)
                {
                    ...
                    sourceNode.GeneratePacketAndSent(Convert.ToInt16(comb_number_of_packets.Text), true, Controls[0], Controls[1], Controls[2], Controls[3]);
                ...
                else
                {
                    ...
                    // start: send data:
                    Sensor sourceNode = _mianWinsow.myNetWork[Convert.ToInt16(comb_source_node.Text)];
                    sourceNode.GeneratePacketAndSent(Convert.ToInt16(comb_number_of_packets.Text), false, Settings.Default.EnergyDistCnt, Settings.Default.TransDistanceDistCnt, Settings.Default.DirectionDistCnt, Settings.Default.PrepDistanceDistCnt);
                }
       ...
       /// <param name="e"></param>
        ...
        private void btn_show_comhansiveResults_Click(object sender, RoutedEventArgs e)
        {
            ...
            for (int i = 0; i < numberOfExp; i++)
            {
                ...
                Dat.ZoneWidth = Settings.Default.ZoneWidthCnt;
            ...
            if (ListData.Count > 0)
            {
                ...
                averageAll.ZoneWidth = Settings.Default.ZoneWidthCnt;
