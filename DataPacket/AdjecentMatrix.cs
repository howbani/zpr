...
namespace ProZoneRouting.DataPacket
{
    ...
    public class AdjecentMatrix
    {
        ...
        {
            int n = N.Count;
            double[,,] M = new double[Parameters.PublicParamerters.PackeTSequenceID,n,n];
            List<RoutingRandomVariable> L = new List<RoutingRandomVariable>();
            foreach(Sensor s in N) { L.AddRange(s.HistoricalRelativityRows); }

            for (int x = 0; x < L.Count; x++)
            {
                RoutingRandomVariable row = L[x];
                int k = System.Convert.ToInt32(row.PID)-1; // the path is starting from the 1.
                int i = row.Fid;
                int j = row.Nid;
                M[k, i, j] = row.RoutingVariableProb;
            }
